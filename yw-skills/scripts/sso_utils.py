...
DEFAULT_APP_URL = "http://szdz.cdmbc.cn:8090"
DEFAULT_APP_CODE = "a1234cd22f9f3647417f71a1e3bc03d6"
DEFAULT_LOGIN_DEVICE_INFO = "a8799d06d5e4b4bf4b8efe54b5086f7c"


def get_config_path():
    home_dir = os.path.expanduser("~")
    return os.path.join(home_dir, ".yw-skills", "config.yml")
                ...
                if content:
                    ...
                    if config and isinstance(config, dict):
                        return {
                            ...
                            'sso_token': config.get('sso_token'),
                            'auth_token_key': config.get('auth_token_key'),
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="yw-skills SSO登录工具")
        ...
        if resp_code == 200:
            ...
            if datas:
                ...
                if token:
                    ...
                    current_config['sso_token'] = token
                    current_config['user_id'] = user_id
                    save_config(current_config)
                    print(f"已保存token: {token[:20]}...")
        else:
